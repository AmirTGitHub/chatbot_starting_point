...
(function ($) {
  ...
  var dialogFlowApiKey = "";

  // Session details
  var sessionId = null; // always null when page loads...
  ...
  var templates = {
    text: {
      user:
        '<li><div class="message message-user">{{speech}}</div><div class="message-data align-right"><span class="message-data-time">{{time}}</span></div></li>',
      bot:
        '<li><div class="message message-bot message-simple">{{speech}}</div><div class="message-data"><span class="message-data-time">{{time}}</span></div></li>',
    },
    card: {
      image: '<img src="{{image}}">',
      title: '<div class="message-card__title">{{title}}</div>',
      description:
        '<div class="message-card__description">{{description}}</div>',
      button: '<button type="button" class="btn">{{text}}</button>',
      reply:
        '<button type="button" class="btn" onclick="getReply(this)">{{text}}</button>',
      link:
        '<button type="button" class="btn" onclick="getLink(\'{{uri}}\')">{{text}}</button>',
    },
  };
  ...
  // Grab all the details from the query string
  for (var variable in params) {
    if (params.hasOwnProperty(variable)) {
      if (params[variable].trim() !== "") {
        processedParams[variable] = params[variable];
        if (variable == "apikey") {
          dialogFlowApiKey = params[variable];
        }
      }
    }
  }
   ...
   */
  var getNow = function () {
    return moment().format("MMM Do, HH:mm");
  };
   ...
   * @return {undefined}
   */
  var submitUserMessage = function (input) {
    if (input.val().trim() !== "") {
      input.val(input.val().trim()); // remove the enter if it has one!
      displayUserMessage(input.val());
      getResponse(input.val());
      input.val("");
      scrollChatBottom();
    }
  };
  ...
  var displayUserMessage = function (message) {
    var userStringHtml = templates.text.user
      .replace("{{speech}}", message)
      .replace("{{time}}", getNow());
    $("#flmntChat li:last").before(userStringHtml);
  };
  ...
  var scrollChatBottom = function () {
    setTimeout(function () {
      $("#flmntChatConversation").scrollTop(
        $("#flmntChatConversation")[0].scrollHeight
      );
      return false;
    }, 100);
  };
  ...
  /**
   * Set the session Id;
   * @param {object} result from Dialogflow
   * @return {string} sessionId
   */
  var setSessionId = function (result) {
    if (result.id) {
      sessionId = result.id;
  ...
  /**
   * Create content card
   * @param {object} node of messages
   * @return {string} message
   */
  var createCard = function (node) {
    var htmlString = "<li>" + '<div class="message-container">';

    if (node.image) {
      htmlString += '<div class="message message-bot message-image">';
      htmlString += templates.card.image.replace("{{image}}", node.image);
      htmlString += "</div>";
    }

    if ((node.title || node.speech) && node.buttons) {
      htmlString += '<div class="message-inc-buttons">';
    }

    if (node.title || node.speech) {
      htmlString += '<div class="message message-bot message-card">';

      if (node.title) {
        htmlString += templates.card.title.replace("{{title}}", node.title);
      }

      if (node.speech) {
        htmlString += templates.card.description.replace(
          "{{description}}",
          node.speech
        );
      }
    ...
    if (node.buttons) {
      htmlString += '<div class="message message-bot message-buttons">';

      node.buttons.forEach(function (button) {
        if (button.uri) {
          htmlString += templates.card.link
            .replace("{{text}}", button.text)
            .replace("{{uri}}", button.uri);
        } else {
          htmlString += templates.card.reply.replace(/{{text}}/g, button);
        }
      });
   ...
   * @param {object} node of resposnes
   * @return {string} message
   */
  var createTemplate = function (node) {
    var tempString = "";
    switch (node.type) {
      case 0:
        if (node.speech.length > 0) {
          tempString = templates.text.bot
            .replace("{{speech}}", node.speech)
            .replace("{{time}}", getNow());
          tempString = tempString.replace(/<a/g, '<a target="_blank"');
        }
        break;
      case 1:
      case 4:
        tempString = createCard(node);
        break;
      case 5:
        tempString = createCard(node);
        break;
   ...
   * @return {promise}
   */
  var appendMessage = function (message, scroll) {
    var deferred = jQuery.Deferred();

    setTimeout(function () {
      $(".loading").css({ display: "none" });
      var formattedTemplate = createTemplate(message);
      $("#flmntChat li:last").before(formattedTemplate);
      if (scroll) {
        scrollChatBottom(); // adds a loading, then scrolls to the bottom so that it can be seen...
      }
      return deferred.resolve();
    }, processedParams.delay || 3500);
  ...
  /**
   * Process result
   * @param {string} message to proces
   * @return {undefined}
   */
  var processResult = function (messages, scroll) {
    var item = messages.shift();
    $(".loading").css({ display: "block" });
    if (scroll) {
      scrollChatBottom(); // this will scroll to the bottom after each part of a response...
    }
    appendMessage(item, scroll).then(function () {
      if (messages.length) {
        processResult(messages, processedParams.autoscroll);
      } else {
  ...
  var createDataArray = function (message) {
    if (!sessionId) {
      sessionId = null;
    }
    ...
    var dataArray = {
      ...
      sessionId: sessionId,
      name: processedParams.botusername,
    };

    // if (processedParams.ebm) {
    //     dataArray = {
    //         botId: processedParams.botid,
    //         message: message,
    //         providerId: processedParams.providerid,
    //         sessionId: sessionId,
    //         Properties: processedParams
    //     };
    // } else {
    //     dataArray = {
    //         lang: 'en',
  ...
  var getResponse = function (message) {
    ...
    var dataArray = createDataArray(message),
      url = processedParams.endpoint,
      headers = {
        "Content-Type": "application/json; charset=UTF-8",
      };

    // if (!processedParams.ebm && processedParams.apikey) {
    //     headers.Authorization = 'Bearer ' + dialogFlowApiKey;
    // }

    // if (processedParams.endpoint) {
    //     url = processedParams.endpoint;
    // }

    if (processedParams.autoscroll) {
      if (processedParams.autoscroll === "false") {
        processedParams.autoscroll = false;
      } else {
        processedParams.autoscroll = true;
      }
    }
    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        var responsePayload;
        if (processedParams.ebm) {
          if (typeof result.bot_response !== "undefined") {
            result.bot_response.forEach(function (item) {
              allMessages.push(item);
            });
            processResult(allMessages, true);
          } else {
            disableWidget("Error: Incorrect response from EBM");
          }
        } else {
          allMessages.push({
            type: 5,
          ...
          // result.result.fulfillment.messages.forEach(function(item) {
          //     if (item.type === 4) {
          //         if (typeof item.payload.responses !== 'undefined') { // Bot manager-generated custom payload
          //             responsePayload = item.payload.responses[0].response;
          //         } else { // Other custom payload
          //             responsePayload = item.payload.response;
          //         }
          //         responsePayload.forEach(function(customItem) {
          //             customItem.type = 5;
          //             allMessages.push(customItem);
          //         });
          //     } else {
          //         allMessages.push(item);
          //     }
      ...
      error: function (error) {
        ...
        } else if (error.responseJSON.status.errorDetails) {
          //  Dialogflow error
          errorMessage += ": " + error.responseJSON.status.errorDetails;
        }
        disableWidget(errorMessage);
      },
    });
  };

  var disableWidget = function (errorMessage) {
    var displayError = processedParams.widgeterror;
    if (errorMessage) {
      displayError = errorMessage;
    }
    $("#flmntError").append(displayError).css({ display: "block" });
    $("#flmntChatInput").prop("disabled", true).attr("placeholder", "");
    $("#submitMessage").prop("disabled", true);
  };

  window.getReply = function (e) {
    var clickedBtn = $(e);
    getResponse(clickedBtn.text());
    displayUserMessage(clickedBtn.text());
    clickedBtn.addClass("btn-clicked");
    $(this).val("");
  };

  window.getLink = function (url) {
    window.open(url);
    $(this).val("");
  };

  var getCorrectTextColor = function (hex) {
    var threshold = 150;
    var hRed = hexToR(hex);
    ...
    if (cBrightness > threshold) {
      ...
      return "#FFFFFF"; // no idea why this should be set to white... so hard to read.
    }
  };

  var colouriseWidget = function (colourise) {
    var css =
      "" +
      ".loading-graphic span:nth-child(1) { background-color: rgba(" +
      colourise +
      ", 0.2); }" +
      ".loading-graphic span:nth-child(2) { background-color: rgba(" +
      colourise +
      ", 0.5); }" +
      ".loading-graphic span:nth-child(3) { background-color:rgba(" +
      colourise +
      ", 1); }";
  ...
  var autoExpand = function (field) {
    ...
    if (height < 107) {
      ...
      $(".flmntChatFooter").height(height + 16 + "px"); // set the container height..
    } else {
      height = 106;
      field.style.height = height + "px";
      $(".flmntChatFooter").height(height + 16 + "px"); // set the container height..
  ...
  var shrink = function (field) {
    field.style.height = "34px";
    $(".flmntChatFooter").height("50px");
  };

  $(document).ready(function () {
    if (typeof processedParams.primarycolour !== "undefined") {
      colouriseWidget(processedParams.primarycolour);
    }
    if (processedParams.startintent && processedParams.startintent.length > 0) {
      getResponse(processedParams.startintent);
    }
    if (!processedParams.apikey && !processedParams.ebm) {
      disableWidget("Error: no API Key & EBM is not true");
    }
    if (
      processedParams.ebm &&
      (!processedParams.botid || !processedParams.providerid)
    ) {
      disableWidget("Error: no EBM BotID or ProviderID provided");
    }
    $("#flmntChatTitle").text(processedParams.widgettitle);
    $("#flmntChatInput").attr("placeholder", processedParams.widgetplaceholder);
    $("#flmntChatInput").focusin(function () {
      $(this).attr("placeholder", "");
    });
    $("#flmntChatInput").focusout(function () {
      $(this).attr("placeholder", processedParams.widgetplaceholder);
    });

    $(".loading").css({ display: "none" });
    $("#submitMessage").on("click", function (e) {
      e.preventDefault();
      submitUserMessage($("#flmntChatInput"));
      shrink(e.target);
    });
    $("#flmntChatInput").keydown(function (e) {
      if (e.keyCode == 13) {
        e.preventDefault();
        submitUserMessage($(this));
        shrink(e.target);
      } else {
        autoExpand(e.target);
      }
    ...
    var intro = $(".flmntIntroText p");
    if (processedParams.intro != null) {
      intro.text(processedParams.intro);
    }

    $("#flmntChatConversation").scroll(function () {
      setTimeout(function () {
        if (
          $("#flmntChatConversation").scrollTop() +
            $("#flmntChatConversation").height() ==
          $("#flmntChatConversation")[0].scrollHeight
        ) {
          // reached the bottom....
          $(".scrollDown").fadeOut();
    ...
    $(".scrollDown").click(scrollChatBottom);
  });
})(jQuery);
